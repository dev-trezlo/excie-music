...
from ShrutixMusic.utils.inline.settings import (
    auth_users_markup,
    playmode_users_markup,
    setting_markup,
    vote_mode_markup,
)
from ShrutixMusic.utils.inline.start import private_panel
...
async def settings_mar(client, message: Message, _):
    ...
    await message.reply_text(
        _["setting_1"].format(nand.mention, message.chat.id, message.chat.title),
        reply_markup=InlineKeyboardMarkup(buttons),
    )


@nand.on_callback_query(filters.regex("settings_helper") & ~BANNED_USERS)
@languageCB
...
@nand.on_callback_query(filters.regex("settingsback_helper") & ~BANNED_USERS)
@languageCB
...
async def send_repo(_, message: Message):
    await message.reply_video(
        ...
        caption=(
            ...
            "🧡 ᴄʀᴇᴅɪᴛꜱ : <a href='https://t.me/exizemusicupdate'>@exizemusicupdate</a>"
        ),
...
@nand.on_callback_query(filters.regex(pattern=r"^(AUTH|AUTHLIST)$") & ~BANNED_USERS)
@ActualAdminCB
